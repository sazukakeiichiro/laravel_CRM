...
<template>
            ...
            <table class="table-auto w-full text-left whitespace-no-wrap">
              <thead>
                <tr>
                  ...
                  <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">企業名</th>
              ...
              <tbody>
                <tr v-for="customer in customers.value.data" :key="customer.id">
                  <td class="px-4 py-3">
                     <button type="button" @click="setCustomer setCustomer({id: customer.id, kana:customer.kana})" class="text-blue-400">
                    {{ customer.id }}
                  </button>
                </td>
  ...
  <button @click="searchCustomers" data-micromodal-trigger="modal-1" class="flex mx-auto text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg"> 検索</button>
</template> 